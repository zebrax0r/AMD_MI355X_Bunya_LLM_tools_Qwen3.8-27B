#!/usr/bin/env bash
#
# serve-qwen38-27b.sh — one-click Qwen3.8-27B serving on ONE AMD MI355X GPU
# (gfx950, CDNA4, 288 GB HBM3E) on UQ's Bunya supercomputer, via SGLang inside
# an Apptainer container.
#
# ── WHY 27B, AND NOT THE 2.4T ───────────────────────────────────────────────
#
# The sibling repo (AMD_MI355X_Bunya_LLM_tools_Qwen3.8_MXFP4) serves
# Qwen3.8-2.4T-A95B across all eight GPUs of an MI355X node. Getting all eight
# in one allocation is hard, so this repo answers a different question: what is
# the best Qwen3.8 you can serve on a SINGLE card?
#
# The answer is 27B, and it is not a compromise chosen for convenience — it is
# the only other size that exists. Qwen shipped exactly two members of the 3.8
# family: Qwen3.8-27B (dense, 27.8 B params) and Qwen3.8-2.4T-A95B (MoE). There
# is nothing between them. Every "in between" checkpoint on the Hub is a pruned
# 2.4T, and none of them lands in 288 GB in a state worth serving:
#
#   amd/…-2.4T-A95B-Quark-MXFP4              1278 GiB   needs ~6 GPUs
#   RedHatAI/…-NVFP4-REAP-25                 1066 GiB
#   nota-ai/…-NVFP4-Global-Pruned-40          961 GiB   (and NVFP4 = Blackwell only)
#   mgoin/…-NVFP4-pruned75                    406 GiB
#   mgoin/…-NVFP4-pruned94                    170 GiB   author: "basically produces gibberish"
#
# So: 27B on one card, with the whole card to itself.
#
# ── WHY THE AMD MXFP4 CHECKPOINT ────────────────────────────────────────────
#
# Default is amd/Qwen3.8-27B-Quark-AWQ-MXFP4 — 18.4 GiB, OCP MXFP4 W4A4 (group
# 32, E8M0 scales), which is precisely the format gfx950 has silicon for. AMD
# report GSM8K recovery of 101.8% (thinking) / 99.2% (non-thinking) vs the BF16
# base, so this is not a quality trade in any measurable sense.
#
# Two details of that checkpoint matter operationally, both verified by reading
# its safetensors header directly:
#   * the vision tower is EXCLUDED from quantisation and stays BF16 (333
#     tensors, 0.86 GiB) — so this is still a full VLM, not a text-only stub
#   * the MTP head ships inside it in BF16 (15 tensors) — so NEXTN speculative
#     decoding costs no extra download and no extra weights
#
# The BF16 and FP8 checkpoints are plumbed as fallbacks; './serve-qwen38-27b.sh
# check' probes all three and prints sizes before you commit to a download.
#
# Usage:
#   ./serve-qwen38-27b.sh [serve]  start the server (default; runs until killed)
...
#   ./serve-qwen38-27b.sh pull     build the .sif from the container image (once)
#   ./serve-qwen38-27b.sh check    can this image load this model? (arch vs
#                                registry, plus the checkpoint ladder + sizes)
#   ./serve-qwen38-27b.sh gpucheck can this image reach this node's GPUs? (~1 min)
#   ./serve-qwen38-27b.sh budget   HBM arithmetic for the current settings — how
#                                many concurrent requests, at what context
#   ./serve-qwen38-27b.sh toolcheck tool-call + reasoning round trip vs a server
#   ./serve-qwen38-27b.sh vlmcheck  image round trip vs a running server
#   ./serve-qwen38-27b.sh speccheck is speculative decoding SAFE on this image?
#   ./serve-qwen38-27b.sh parsers  list tool-call/reasoning parsers this image has
#   ./serve-qwen38-27b.sh loadstat why the last cold start was slow (reads the log)
#   ./serve-qwen38-27b.sh download prefetch model weights only (no GPU needed)
#   ./serve-qwen38-27b.sh stop     stop a running server
#   ./serve-qwen38-27b.sh status   show server state + health endpoint
#
# Unlike the 2.4T bundle, 'check' before 'download' is cheap advice rather than
# a 1.4 TB gate — the default checkpoint is 18.4 GiB. Run it anyway; it is the
# thing that tells you whether the image can build the model at all.
#
# Apptainer lives ONLY on Bunya compute nodes, never the login nodes — so every
# mode except a bare 'stop'/'status'/'budget' must run inside an allocation.
#
# Configuration comes from qwen38-27b.env next to this script (or $Q27X_ENV),
# see qwen38-27b-env.example. Environment variables you export beforehand win.
...
log()  { printf '\033[1;34m[q27x]\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33m[q27x WARN]\033[0m %s\n' "$*" >&2; }
die()  { printf '\033[1;31m[q27x ERROR]\033[0m %s\n' "$*" >&2; exit 1; }
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    warn "No config file at $ENV_FILE (copy qwen38-27b-env.example to qwen38-27b.env); using environment only."
fi

# ── Shadowed-config detector ────────────────────────────────────────────────
#
# Every line in qwen38-27b-env.example is `export VAR="${VAR:-default}"` so that a
...
#
#   $ source qwen38-27b.env          # exports MODEL_ID=<old value> into your shell
#   $ vi qwen38-27b.env              # change MODEL_ID
#   $ source qwen38-27b.env          # ${MODEL_ID:-<new>} sees the OLD export...
#   $ ./serve-qwen38-27b.sh check    # ...so your edit is silently discarded
#
...
# value here is expensive: it points the download at the wrong repo, or
...
#
# NOTE you do not need to source qwen38-27b.env to use these scripts at all; they
...
# AMD's own Quark AWQ-MXFP4 build of Qwen3.8-27B. 18.4 GiB, OCP MXFP4 W4A4
# (group 32, E8M0 scales) — the format gfx950 has silicon for. Vision tower and
# MTP head stay BF16 inside it, so this is a full VLM and NEXTN speculative
# decoding needs no extra download. See './serve-qwen38-27b.sh check'.
MODEL_ID="${MODEL_ID:-amd/Qwen3.8-27B-Quark-AWQ-MXFP4}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen3.8-27b}"

# The checkpoint ladder, fastest first. 'check' probes each for reachability,
# size and declared quant_method so you pick a working MODEL_ID before the
# download rather than after. Measured sizes (Aug 2026):
#
#   amd/Qwen3.8-27B-Quark-AWQ-MXFP4   18.4 GiB  MXFP4 W4A4, native gfx950
#   Qwen/Qwen3.8-27B-FP8              28.7 GiB  FP8, also native on CDNA4
#   Qwen/Qwen3.8-27B                  51.7 GiB  BF16 reference; always works
#
# All three fit one 288 GB card with room to spare — the ladder is about decode
# BANDWIDTH, not capacity. Decode is memory-bound, so weight bytes read per
# token is the number that sets your token rate. See the README.
MODEL_CANDIDATES="${MODEL_CANDIDATES:-amd/Qwen3.8-27B-Quark-AWQ-MXFP4 Qwen/Qwen3.8-27B-FP8 Qwen/Qwen3.8-27B}"

SGLANG_IMAGE="${SGLANG_IMAGE:-docker://lmsysorg/sglang-rocm:v0.5.18-rocm720-mi35x-20260822}"
PORT="${PORT:-30000}"
# ONE GPU. That is the entire point of this repo. TP_SIZE>1 works and is not
# forbidden — if you happen to hold more of the node, raise it — but nothing
# here needs it: 18.4 GiB of weights on a 288 GB card is a 6.4% occupancy.
TP_SIZE="${TP_SIZE:-1}"
DP_SIZE="${DP_SIZE:-1}"
CONTEXT_LEN="${CONTEXT_LEN:-}"
# A number, or the literal 'auto' to pass no --mem-fraction-static at all and
# let SGLang derive one. The two are NOT equivalent, and the difference is
# larger than it looks: adjust_mem_fraction_for_vlm() — the ~7.5% haircut for
# the vision tower — lives INSIDE SGLang's `if mem_fraction_static is None`
# branch, so an explicit value SKIPS it. Passing 0.9 therefore gives you MORE
# usable HBM than leaving it on auto, not less.
# './serve-qwen38-27b.sh budget' models both.
MEM_FRACTION="${MEM_FRACTION:-0.9}"
# EMPTY on purpose. trtllm_mha is SM100-only, and on gfx950 the model hook picks
# the right backend itself. Setting one here is how you quietly opt out of that
# — and on a VLM it also changes the memory budget, because SGLang applies an
# extra 0.85 to --mem-fraction-static when the backend resolves to 'aiter' and
# context > 8192. './serve-qwen38-27b.sh budget' shows the resulting arithmetic.
ATTENTION_BACKEND="${ATTENTION_BACKEND:-}"
QUANTIZATION="${QUANTIZATION:-}"
CUDA_GRAPH_MAX_BS_DECODE="${CUDA_GRAPH_MAX_BS_DECODE:-}"
DISABLE_RADIX_CACHE="${DISABLE_RADIX_CACHE:-0}"
# Empty = BF16 KV (64 KiB/token). 'fp8_e4m3' halves that to 32 KiB/token and is
# native on CDNA4. With 27B on a whole card KV is rarely the binding constraint
# — the GDN state pool usually is — so this stays off by default and 'budget'
# tells you whether turning it on would actually buy you anything.
...
# ── Multimodal ──────────────────────────────────────────────────────────────
# This checkpoint is a VLM (Qwen3_5ForConditionalGeneration) with a BF16 SigLIP-
# style tower: 27 blocks, hidden 1152, patch 16, spatial merge 2. Vision is ON
# by default. Set ENABLE_MULTIMODAL=0 for a text-only server, which skips the
...
SPEC_GATE="${SPEC_GATE:-on}"
DSPARK_MODEL="${DSPARK_MODEL:-RadixArk/Qwen3.8-27B-DSpark}"
# Unpinned by default. The K3 repo pinned its draft because RadixArk rewrote it
# three times in four days, twice breaking the launch. This one has no history
...
# ── ReplaySSM: two different flags, mutually exclusive ───────────────────────
#
# SGLang has two ReplaySSM paths over the GDN recurrent state, and they share
# ring storage with incompatible cursor protocols — server_args.py rejects the
# pair outright. Both require the Triton linear-attn decode backend, which is
# what ROCm uses anyway, so BOTH are available here (unlike the CUDA-only
# flashinfer/cutedsl linear-attention backends).
#
#   REPLAYSSM_SPEC=1  --enable-linear-replayssm-spec
...
#       same ratio. Needs a speculative algorithm and a linear chain (topk<=1);
#       NEXTN qualifies. Verify output is unchanged — a memory trade, not an
#       accuracy one. This is the one that composes with the default.
#
#   REPLAYSSM_DECODE=1  --enable-linear-replayssm
#       buffered DECODE. Upstream claims ~1.2-1.5x GDN decode bandwidth at
#       batch >= 64. Requires --mamba-radix-cache-strategy no_buffer, and
#       CANNOT be combined with speculative verify. This is the throughput-first
#       branch: set SPECULATIVE=none with it.
REPLAYSSM_SPEC="${REPLAYSSM_SPEC:-1}"
REPLAYSSM_DECODE="${REPLAYSSM_DECODE:-0}"
REPLAYSSM_CACHE_LEN="${REPLAYSSM_CACHE_LEN:-}"

# GDN (Gated DeltaNet) state pool. 48 of Qwen3.8-27B's 64 layers are linear
# attention holding a fixed-size recurrent state per request; these size and
# shape that pool. On a single card with only 18.4 GiB of weights, THIS is what
# caps concurrency, not KV — see './serve-qwen38-27b.sh budget'.
# All empty by default — the argv is unchanged until one is set.
...
# heuristic fallback bench-qwen38-27b.sh warns about. Off means the first run is
...
# DEFAULTED, unlike the 2.4T bundle. With speculative decoding on, SGLang's
# speculative hook pins --max-running-requests to 48 when you leave this unset —
# not to anything derived from memory. Since NEXTN is the default here, leaving
# it empty would mean the out-of-the-box server quietly caps concurrency at 48
# on a card with room for far more.
#
# 64 is chosen from the arithmetic, not from taste: at 5 GDN slots per request
# that is 47.3 GiB of state, leaving ~175 GiB of KV — about 45k tokens per
# request at BF16 KV. './serve-qwen38-27b.sh budget' prints the whole table and
# marks this row. Raise it for many short conversations, lower it for few long
# ones; the trade is per-request context, and it is linear.
...
# `FileNotFoundError: [Errno 2] No such file or directory: ''`. qwen38-27b-env.example
...
# Measured from amd/Qwen3.8-27B-Quark-AWQ-MXFP4's safetensors header: 18.4 GiB
# = 20 GB (12.05 GiB packed MXFP4 + scales, 6.39 GiB BF16 for embeddings,
# lm_head, norms, the vision tower and the MTP head). The other rungs are 31 GB
# (FP8) and 56 GB (BF16). Used for the effective-GB/s readout, the free-space
# preflight and the presharded space check — 'check' prints the real number per
# candidate, so change this if you change MODEL_ID.
WEIGHTS_GB="${WEIGHTS_GB:-20}"

[[ "$MEM_FRACTION" == "auto" || "$MEM_FRACTION" =~ ^[0-9]*\.?[0-9]+$ ]] \
    || die "MEM_FRACTION must be a number or 'auto', got '$MEM_FRACTION'."

case "$SPEC_GATE" in
    on|off) ;;
    *) die "SPEC_GATE must be 'on' or 'off', got '$SPEC_GATE'." ;;
esac
...
[[ "$WEIGHTS_GB" =~ ^[0-9]+$ && "$WEIGHTS_GB" -gt 0 ]] \
  ...
  './serve-qwen38-27b.sh check' prints the real size of each checkpoint candidate."
...
# once. This checkpoint is ONE 18.4 GiB safetensors file, so the thread count
# buys much less here than it does for the 213-shard 2.4T — a single file is a
# single reader no matter what you set. It is left plumbed because the FP8 and
# BF16 rungs of the ladder ARE sharded (66 and 18 files), and there it matters.
...
# DSPARK-only flags. Fail here rather than after the weights are resident.
...
# ── The two ReplaySSM flags are mutually exclusive ──────────────────────────
# server_args.py raises on the pair: "they share the ring storage but drive it
# with incompatible cursor protocols (per-decode-forward vs per-verify-commit
# advance)". Catch it here so the message names OUR variables, not SGLang's.
if [[ "$REPLAYSSM_DECODE" == "1" && "$REPLAYSSM_SPEC" == "1" ]]; then
    die "REPLAYSSM_DECODE=1 and REPLAYSSM_SPEC=1 cannot both be set — SGLang rejects
  --enable-linear-replayssm together with --enable-linear-replayssm-spec.
  Pick the branch that matches what you are optimising:
    latency    SPECULATIVE=nextn  REPLAYSSM_SPEC=1  REPLAYSSM_DECODE=0  (default)
    throughput SPECULATIVE=none   REPLAYSSM_SPEC=0  REPLAYSSM_DECODE=1
               MAMBA_RADIX_STRATEGY=no_buffer"
fi

# Buffered ReplaySSM DECODE has two hard requirements, both of which produce a
# startup failure rather than a slow server, so check them up front.
if [[ "$REPLAYSSM_DECODE" == "1" ]]; then
    if [[ -n "$SPECULATIVE" ]]; then
        die "REPLAYSSM_DECODE=1 is a plain-decode optimisation and does not run under
  speculative decoding — the verify path owns the ring instead. You have
  SPECULATIVE=$SPECULATIVE. Set SPECULATIVE=none, or use REPLAYSSM_SPEC=1
  (the spec-verify variant) instead."
    fi
    # mamba_extra_buffer_of() must be false. 'auto' resolves to extra_buffer for
    # this architecture, so leaving the strategy empty is NOT sufficient.
    case "$MAMBA_RADIX_STRATEGY" in
        no_buffer) ;;
        ""|auto)
            MAMBA_RADIX_STRATEGY="no_buffer"
            log "REPLAYSSM_DECODE=1 — setting MAMBA_RADIX_STRATEGY=no_buffer (required;
  'auto' resolves to extra_buffer for this model, which SGLang rejects here).
  Side effect, and a welcome one: 3 GDN state slots per request instead of 5."
            ;;
        *)
            die "REPLAYSSM_DECODE=1 requires MAMBA_RADIX_STRATEGY=no_buffer, got
  '$MAMBA_RADIX_STRATEGY'. The extra_buffer ping-pong donation path does not
  reset the ReplaySSM ring cursor, so SGLang refuses the combination."
            ;;
    esac
    [[ -z "$LINEAR_ATTN_BACKEND" || "$LINEAR_ATTN_BACKEND" == "triton" ]] \
        || die "REPLAYSSM_DECODE=1 requires the Triton linear-attn decode backend, got
  LINEAR_ATTN_BACKEND=$LINEAR_ATTN_BACKEND. On ROCm Triton is the only linear-
  attention backend anyway (flashinfer/cutedsl/nvidia_kda are CUDA-gated), so
  leave this empty."
fi

[[ -z "$REPLAYSSM_CACHE_LEN" || "$REPLAYSSM_CACHE_LEN" =~ ^[0-9]+$ ]] \
    || die "REPLAYSSM_CACHE_LEN must be a positive integer or empty, got '$REPLAYSSM_CACHE_LEN'."

case "$ENABLE_MULTIMODAL" in
    0|1) ;;
    *) die "ENABLE_MULTIMODAL must be 0 or 1, got '$ENABLE_MULTIMODAL'." ;;
esac
...
if [[ -n "$MODEL_CACHE_DIR" ]]; then
    PID_FILE="${PID_FILE:-$MODEL_CACHE_DIR/q27x-server.pid}"
    LOG_FILE="${LOG_FILE:-$MODEL_CACHE_DIR/q27x-server.log}"
    LOADTIMES_FILE="${LOADTIMES_FILE:-$MODEL_CACHE_DIR/q27x-loadtimes.log}"
else
    PID_FILE="${PID_FILE:-$SCRIPT_DIR/q27x-server.pid}"
    LOG_FILE="${LOG_FILE:-$SCRIPT_DIR/q27x-server.log}"
    LOADTIMES_FILE="${LOADTIMES_FILE:-$SCRIPT_DIR/q27x-loadtimes.log}"
fi
...
case "$MODE" in
    ...
    status)
        ...
        if curl -fsS -m 5 "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then
            ...
            key=""
            [[ -r "${MODEL_CACHE_DIR:-}/q27x-api-key" ]] && key="$(<"$MODEL_CACHE_DIR/q27x-api-key")"
            curl -fsS -m 5 "http://127.0.0.1:${PORT}/v1/models" \
                 -H "Authorization: Bearer ${Q27X_API_KEY:-$key}" 2>/dev/null | head -c 400 || true
            echo
        else
...
def post(payload):
    ...
    try:
        ...
        print(f"  request failed: {e}\n  Is the server up? ./serve-qwen38-27b.sh status")
        sys.exit(1)
...
if not reasoning0 or "<think>" in content0:
    ...
    print("  Check './serve-qwen38-27b.sh parsers' — it should be 'qwen3'.")
...
if not calls:
    ...
    print("  matching this model. Check './serve-qwen38-27b.sh parsers' and TOOL_PARSER")
...
PY
        ...
        ;;
    budget)
        # Pure arithmetic. No container, no GPU, no allocation — run it on a
        # login node while you decide what to ask SLURM for.
        #
        # Everything here comes from the checkpoint's own config.json and from
        # SGLang's own memory accounting, not from rules of thumb. On a single
        # card the two numbers that matter are (a) how much of the 288 GB SGLang
        # actually hands to the static pool, and (b) how that pool splits
        # between KV and the GDN recurrent state.
        HBM_GB="${HBM_GB:-288}"
        MF="$MEM_FRACTION" \
        HBM="$HBM_GB" \
        WGB="$WEIGHTS_GB" \
        KVD="${KV_CACHE_DTYPE:-auto}" \
        MM="$ENABLE_MULTIMODAL" \
        ATTN="${ATTENTION_BACKEND:-auto}" \
        CTX="${CONTEXT_LEN:-262144}" \
        STRAT="${MAMBA_RADIX_STRATEGY:-auto}" \
        RADIX_OFF="$DISABLE_RADIX_CACHE" \
        MRR="${MAX_RUNNING_REQUESTS:-}" \
        python3 - <<'BUDGETPY'
import os

explicit = os.environ["MF"] not in ("", "auto")
mf   = float(os.environ["MF"]) if explicit else 0.88
hbm  = float(os.environ["HBM"])
wgb  = float(os.environ["WGB"])
kvd  = os.environ["KVD"]
mm   = os.environ["MM"] == "1"
attn = os.environ["ATTN"]
ctx  = int(os.environ["CTX"])
strat = os.environ["STRAT"]
radix_off = os.environ["RADIX_OFF"] == "1"
mrr  = os.environ["MRR"]

GiB = 1024 ** 3

print("Qwen3.8-27B on one MI355X — HBM budget")
print("=" * 70)
print("  Card                     %.0f GB HBM3E (gfx950, CDNA4)" % hbm)
print("  --mem-fraction-static    %s" % (mf if explicit else "auto (SGLang derives ~0.88, then cuts it)"))

factor = mf
notes = []

# ── The VLM haircut applies ONLY when --mem-fraction-static is unset ────────
# adjust_mem_fraction_for_vlm() is called from _handle_gpu_memory_settings(),
# INSIDE the `if self.mem_fraction_static is None:` branch. So passing an
# explicit fraction — which this repo does, at 0.9 — bypasses it entirely.
# That is worth knowing in its own right: setting it explicitly gives you MORE
# memory than leaving it on auto, not less.
if mm and explicit:
    notes.append("VLM haircut          NOT applied — MEM_FRACTION is set explicitly")
    notes.append("    (%s). adjust_mem_fraction_for_vlm() lives inside SGLang's" % mf)
    notes.append("    `if mem_fraction_static is None` branch, so an explicit value")
    notes.append("    skips it. Leave MEM_FRACTION empty and SGLang derives a value")
    notes.append("    and THEN cuts it ~7.5% for the vision tower.")
elif mm:
    # The auto path. base 0.95, scaled by ViT complexity vs ViT-L/14. NOTE it
    # reads vision_config.num_hidden_layers, which THIS config does not define
    # (it uses "depth": 27), so the ViT-L/14 default of 24 applies.
    vit_layers, vit_hidden = 24, 1152
    ratio = (vit_layers * vit_hidden ** 2) / (24 * 1024 ** 2)
    dyn = max(0.8, min(1.05, 1.0 - 0.1 * (ratio - 1.0)))
    vitf = 0.95 * dyn
    factor *= vitf
    notes.append("VLM haircut          x %.4f  (MEM_FRACTION left to SGLang)" % vitf)
    notes.append("    ViT complexity ratio %.4f vs ViT-L/14. SGLang reads" % ratio)
    notes.append("    vision_config.num_hidden_layers, which this config does not")
    notes.append("    set — it uses \"depth\": 27 — so SGLang falls back to 24 layers")
    notes.append("    and the haircut is milder than the real tower would justify.")

# The aiter reduction is NOT in that branch — it applies to an explicit value too.
if attn == "aiter" and ctx > 8192:
    factor *= 0.85
    notes.append("aiter, ctx>8192      x 0.85  (applies even to an explicit fraction)")
elif attn == "auto":
    notes.append("aiter x0.85          NOT applied here — ATTENTION_BACKEND is unset,")
    notes.append("    so this is the OPTIMISTIC figure. If the model hook resolves the")
    notes.append("    backend to 'aiter' and context > 8192, SGLang multiplies the")
    notes.append("    fraction by a further 0.85 and every number below drops ~15%.")
    notes.append("    Unlike the VLM haircut, this one does apply to an explicit")
    notes.append("    --mem-fraction-static. The server log prints what it settled")
    notes.append("    on — trust that over this.")
...
pool = factor * hbm
print("")
print("  effective fraction       %.4f" % factor)
print("  static pool              %.1f GB" % pool)
print("  - weights                %.1f GB" % wgb)
avail_gb = pool - wgb
print("  = KV + GDN state         %.1f GB" % avail_gb)
if avail_gb <= 0:
    print("")
    print("  NEGATIVE — the weights do not fit the static pool. Raise MEM_FRACTION")
    print("  or pick a smaller checkpoint.")
    raise SystemExit(1)
avail = avail_gb * 1e9 / GiB

# 16 of 64 layers are full attention (full_attention_interval: 4);
# 4 KV heads x head_dim 256, K and V.
kv_bytes = 16 * 2 * 4 * 256 * (1 if kvd.startswith("fp8") else 2)
print("")
print("  KV cache dtype           %s%s" % (kvd, "  (BF16)" if kvd in ("auto", "bf16", "bfloat16") else ""))
print("  KV per token             %.0f KiB   (16 full-attn layers x 4 KV heads x 256)" % (kv_bytes / 1024))

conv = 10240 * 4            # conv_dim x kernel width
ssm = 48 * 128 * 128        # v_heads x v_dim x k_dim
gdn_slot = (conv + ssm) * 4 # float32
print("  GDN per state slot       %.1f MiB   (48 linear layers x 3.156 MiB, fp32)" % (gdn_slot * 48 / 2 ** 20))

if radix_off:
    slots, why = 1, "radix cache OFF"
elif strat == "no_buffer":
    slots, why = 3, "no_buffer"
elif strat == "extra_buffer_lazy":
    slots, why = 4, "extra_buffer_lazy"
else:
    slots, why = 5, "extra_buffer ('auto' for this model)"
per_req_gdn = gdn_slot * 48 * slots
print("  GDN per REQUEST          %.3f GiB   (%d slot(s), %s)" % (per_req_gdn / 2 ** 30, slots, why))

print("")
print("  What that buys — pick a row, or set MAX_RUNNING_REQUESTS:")
print("  " + "-" * 68)
print("  %10s  %9s  %9s  %13s  %11s" % ("concurrent", "GDN", "KV left", "total KV", "ctx/req"))
print("  %10s  %9s  %9s  %13s  %11s" % ("requests", "GiB", "GiB", "tokens", "tokens"))
print("  " + "-" * 68)
rows = [8, 16, 32, 64, 128, 192, 256]
if mrr.isdigit() and int(mrr) not in rows:
    rows = sorted(rows + [int(mrr)])
for n in rows:
    g = per_req_gdn * n / 2 ** 30
    kvleft = avail - g
    if kvleft <= 0:
        print("  %10d  %9.1f  %9s  %s" % (n, g, "--", "GDN pool alone exceeds the budget"))
        continue
    toks = int(kvleft * 2 ** 30 / kv_bytes)
    per = toks // n
    # The model's window is 262,144. A row offering more per request than that
    # is not offering you anything — cap it and say so, rather than printing a
    # number nobody can reach.
    cap = ""
    if per > ctx:
        per, cap = ctx, "*"
    mark = "  <- MAX_RUNNING_REQUESTS" if (mrr.isdigit() and n == int(mrr)) else ""
    print("  %10d  %9.1f  %9.1f  %13s  %10s%1s%s"
          % (n, g, kvleft, format(toks, ","), format(per, ","), cap, mark))
print("  " + "-" * 68)
print("  * capped at the model's %s-token window; the KV pool would allow more."
      % format(ctx, ","))

print("""
  Read it this way. GDN state is charged PER REQUEST and is the same whether
  that request holds 100 tokens or 100,000 — it is O(1) in context, which is
  the whole point of a hybrid backbone. KV is charged per TOKEN. So the left
  column buys concurrency, and the right column is what each of those
  concurrent requests can actually hold.

  Levers, in the order worth trying:
    MAX_RUNNING_REQUESTS=<n>        pick the row you want, explicitly
    KV_CACHE_DTYPE=fp8_e4m3         doubles every 'ctx/req' figure above;
                                    native on CDNA4, not emulated
    MAMBA_RADIX_STRATEGY=no_buffer  5 GDN slots per request -> 3
    CONTEXT_LEN=<n>                 caps one request; does NOT resize the pools
""")
if mm and not explicit:
    print("  ENABLE_MULTIMODAL=0 drops the VLM haircut entirely (text-only server).")
    print("")
elif mm:
    print("  ENABLE_MULTIMODAL=0 serves text only. It frees the 0.86 GiB BF16 tower,")
    print("  but with MEM_FRACTION set explicitly there is no VLM haircut to drop.")
    print("")
BUDGETPY
        exit 0
        ;;
    vlmcheck)
        # Image round trip against a RUNNING server. Pure HTTP — no .sif, no
        # GPU — so it works through a tunnel, like toolcheck.
        #
        # The image is GENERATED here in stdlib rather than fetched: a check
        # that depends on the compute node reaching the internet is a check that
        # fails for the wrong reason. It is a 4x4 checkerboard with exactly one
        # red square, in a corner the model has to actually look at to name.
        #
        # That matters more than it sounds. A VLM whose vision tower failed to
        # load does not raise — it answers confidently and wrong. So this
        # asserts on a value the model cannot infer from the prompt.
        key=""
        [[ -r "${MODEL_CACHE_DIR:-}/q27x-api-key" ]] && key="$(<"$MODEL_CACHE_DIR/q27x-api-key")"
        BASE="${TOOLCHECK_URL:-http://127.0.0.1:$PORT}" \
        KEY="${Q27X_API_KEY:-$key}" \
...
W = H = 128
CELL = W // 4
RED, LIGHT, DARK = (220, 30, 30), (235, 235, 235), (40, 40, 40)

rows = []
for y in range(H):
    row = bytearray(b"\x00")          # PNG filter byte 0 (None) per scanline
    cy = y // CELL
    for x in range(W):
        cx = x // CELL
        if cy == 0 and cx == 3:
            ...
            px = LIGHT if (cx + cy) % 2 == 0 else DARK
        row += bytes(px)
...
print("Generated a %dx%d PNG (%d bytes): 4x4 checkerboard, one RED square top-right."
      % (W, H, len(png)))
...
def post(payload):
    ...
    try:
        ...
        if "multimodal" in body.lower() or "image" in body.lower():
            print("")
            print("  If the server says it has no multimodal support, it was started")
            print("  with ENABLE_MULTIMODAL=0, or MODEL_ID names a text-only build.")
        sys.exit(1)
    except Exception as e:
        print("  request failed: %s" % e)
        print("  Is the server up? ./serve-qwen38-27b.sh status")
        sys.exit(1)
...
r = post({"model": MODEL, "max_tokens": 3072, "temperature": 0,
          "reasoning_effort": "low",
          "messages": [{"role": "user", "content": [
              {"type": "text", "text":
               "This image is a 4x4 checkerboard. Exactly one square is red. "
               "Which corner is the red square in? Answer with exactly one of: "
...
check(bool(content), "server returned content", "finish_reason=%s" % fin)
low = content.lower()
right = ("top-right" in low) or ("top right" in low)
check(right, "named the correct corner (top-right)", repr(content[:200]))
if not right:
    for wrong in ("top-left", "bottom-left", "bottom-right"):
        if wrong in low:
            print("        it said %r — the tower is running but not reading this" % wrong)
            print("        correctly. Check the preprocessor config, and confirm the")
            print("        request reached the model you think it did.")
            break

print("")
print("  answer: %s" % content[:300])
print("")
if fails:
    print("VLM ROUND TRIP FAILED: %d check(s) - %s" % (len(fails), ", ".join(fails)))
    print("")
    print("  A VLM that lost its vision tower does not error; it guesses. Check the")
    print("  'check' output for vision_config, and confirm MODEL_ID is the AWQ-MXFP4")
    print("  build — its tower is EXCLUDED from quantisation and stays BF16, unlike")
    print("  some community quants of the same model that drop vision entirely.")
    sys.exit(1)
print("VLM ROUND TRIP OK - the vision tower is loaded and reading the image.")
VLMPY
        exit $?
        ;;
    serve|pull|download|check|gpucheck|parsers|loadstat|speccheck) ;;
    *)
        die "Unknown mode '$MODE'. Use: serve | pull | download | check | gpucheck | budget |
       toolcheck | vlmcheck | speccheck | parsers | loadstat | stop | status"
        ;;
esac
...
[[ -n "$MODEL_CACHE_DIR" ]] \
    || die "MODEL_CACHE_DIR is not set. Point it at scratch (e.g. /scratch/user/\$USER/qwen38/hf-cache). See qwen38-27b-env.example."
...
pull_image() {
    local logf rc
    logf="$(mktemp "${APPTAINER_TMPDIR:-/tmp}/q27x-pull.XXXXXX.log")"
...
if [[ ! -f "$SIF_PATH" ]]; then
    ...
    pull_image \
        || die "apptainer pull failed. Run './serve-qwen38-27b.sh pull' explicitly to debug."
fi
...
#
# ...while torch sees the GPUs fine, because PyTorch's ROCm wheel bundles its own
...
# crash 30 frames deep in aiter, or worse, a full weight load.
...
if [[ "$MODE" == "loadstat" ]]; then
    ...
    if [[ ! -r "$LOG_FILE" ]]; then
        ...
        if grep -qi "falling back to single-threaded" "$LOG_FILE"; then
            ...
            echo
            log "Fix: set WEIGHT_LOAD_THREADS=8 in qwen38-27b.env and restart."
        else
    ...
    if [[ -r "$LOADTIMES_FILE" ]]; then
        ...
        echo
        log "Compare COLD runs only. The weights are ~20 GB against far more host RAM,"
...
if [[ "$MODE" == "parsers" ]]; then
    ...
    log "Set both in qwen38-27b.env from this list ('none' to omit)."
    exit 0
fi

# ── check mode: is the checkpoint reachable, and can this image load it? ────
# Cheaper here than in the 2.4T bundle — the download it gates is 18.4 GiB, not
# 1.4 TB — but it answers two questions worth knowing before a cold start:
# whether MODEL_ID is fetchable with your token, and whether this image's model
# registry actually contains Qwen3_5ForConditionalGeneration.
...
if [[ "$MODE" == "check" ]]; then
    ...
    log "Checkpoint ladder — reachability, size and declared quant_method:"
    echo
    apptainer exec \
...
PY
  ...
  the file (see the warning above). Run:
      unset MODEL_ID && ./serve-qwen38-27b.sh check"
           else
...
if [[ "$weights_cached" -eq 0 ]]; then
    ...
    log "Weights not cached — first start downloads ~${EST_GB} GB. Run './serve-qwen38-27b.sh download' first."
else
...
# model has finished loading. That is the whole load thrown away to learn a
...
if [[ "$SPECULATIVE" == "dspark" && "$MODE" == "serve" ]]; then
  ...
  Fetch it first:   ./serve-qwen38-27b.sh download
  ...
  Fetch it first:   DSPARK_REVISION=$DSPARK_REVISION ./serve-qwen38-27b.sh download
  Or track the branch instead:   DSPARK_REVISION=main"
    ...
    #   ValueError: Unrecognized model in RadixArk/Qwen3.8-27B-DSpark.
  ...
  Re-fetch it:   rm -rf $draft_dir && ./serve-qwen38-27b.sh download"
    fi
  ...
  The download is incomplete. Re-fetch:   rm -rf $draft_dir && ./serve-qwen38-27b.sh download
  Or serve without speculative decoding:  unset SPECULATIVE"
    fi
...
" "$draft_snapshot/config.json" 2>/dev/null || true)"
  ...
  Re-fetch it:                 rm -rf $draft_dir && ./serve-qwen38-27b.sh download
  Pin a known-good revision:   DSPARK_REVISION=<sha> ./serve-qwen38-27b.sh download
...
if [[ "$MODE" == "download" ]]; then
  ...
  If it failed with 401/403, run './serve-qwen38-27b.sh check' — the candidate table
...
renorm_bindings() {
    if (( ! RENORM_PROBED )); then
        ...
        RENORM_BINDINGS="$(apptainer exec "$SIF_PATH" bash -c '
            ...
            # bound MODEL_CACHE_DIR — tens of GB of weights — to find a 30 KB file.
...
if [[ "$MODE" == "speccheck" ]]; then
    # Is speculative decoding SAFE on this image? Needs the .sif, but no GPU and
    # no weights — it is a textual probe of the image's HIP sampling bindings,
    # so it runs in seconds and answers the question the serve-time gate will
    # ask before you spend a cold start finding out.
    log "Probing $SIF_PATH for the HIP sampling bindings speculative verify needs ..."
    if ! renorm_has probed; then
        warn "Could not read the image's sampling bindings."
        warn "  dflash_utils.py was not found under the image roots this probes."
        warn "  That is 'could not tell', NOT 'unsafe' — the serve-time gate treats"
        warn "  it the same way and lets the launch proceed."
        exit 2
    fi
    ok=1
    if renorm_has k && renorm_has p; then
        log "  top_k renorm kernel : present"
        log "  top_p renorm kernel : present"
    else
        ok=0
        printf '  top_k renorm kernel : %s\n' "$(renorm_has k && echo present || echo MISSING)"
        printf '  top_p renorm kernel : %s\n' "$(renorm_has p && echo present || echo MISSING)"
    fi
    if renorm_has v && ! renorm_has t; then
        ok=0
        echo "  non-greedy verify   : CLAIMED BUT UNBOUND (sglang #33694)"
        echo "                        temperature > 0 kills the server"
    else
        echo "  non-greedy verify   : ok"
    fi
    echo
    if (( ok )); then
        log "SAFE — speculative decoding can run on this image."
        log "  SPECULATIVE=nextn (the default) needs no draft download; the MTP head"
        log "  ships inside the checkpoint."
        exit 0
    fi
    warn "UNSAFE — this image will die on ordinary requests with speculative decoding on."
    warn "  Qwen's recommended sampling for this model is temperature=1.0, top_p=0.95,"
    warn "  top_k=20, so a normal client trips this on the first request and takes the"
    warn "  SERVER down, not just the request."
    warn "  Fix: a newer lmsysorg/sglang-rocm:*-mi35x-* image, or SPECULATIVE=none."
    exit 1
fi
...
if command -v rocminfo >/dev/null 2>&1; then
    ...
    case "$gfx" in
        ...
        gfx942) warn "Detected gfx942 (MI300X/MI325X). CDNA3 has no hardware MX matmul, so
        the default MXFP4 checkpoint will not run — but this model is small enough
        that you do not need it: set MODEL_ID=Qwen/Qwen3.8-27B-FP8 (28.7 GiB, and
        FP8 IS native on CDNA3) or the BF16 build. Both fit one MI300X.
...
if [[ -n "$host_rocm" && -n "$sif_rocm" ]]; then
  ...
  './serve-qwen38-27b.sh gpucheck' if startup fails in aiter."
    fi
fi
...
if [[ -n "$alloc_count" && "$alloc_count" -gt 0 && "$GPUS_USED" -ne "$alloc_count" ]]; then
    warn "TP_SIZE=$TP_SIZE uses $GPUS_USED GPU(s) but $alloc_count are allocated.
  Unlike the 2.4T bundle this is NOT an error — 18.4 GiB of weights fit one card,
  and TP=1 avoids all-reduce entirely. Raise TP_SIZE only if you want the extra
  HBM for KV, and expect the gain to be sublinear."
fi

# Port free?
if (exec 3<>"/dev/tcp/127.0.0.1/$PORT") 2>/dev/null; then
    exec 3>&- 3<&- || true
    die "Port $PORT is already in use on this node (another server running? try './serve-qwen38-27b.sh status')."
fi

if server_running; then
    die "A server is already recorded running (pid $(<"$PID_FILE")). Use './serve-qwen38-27b.sh status' or 'stop' first."
fi

# ── API key ─────────────────────────────────────────────────────────────────

API_KEY_FILE="$MODEL_CACHE_DIR/q27x-api-key"
if [[ -z "${Q27X_API_KEY:-}" ]]; then
    if [[ -r "$API_KEY_FILE" ]]; then
        Q27X_API_KEY="$(<"$API_KEY_FILE")"
        log "Using API key from $API_KEY_FILE"
    else
        Q27X_API_KEY="$(openssl rand -hex 24 2>/dev/null || head -c 24 /dev/urandom | od -An -tx1 | tr -d ' \n')"
        (umask 077 && printf '%s' "$Q27X_API_KEY" > "$API_KEY_FILE")
...
#
# ...and it dies *after* the full weight load, which is a slow way to
...
if [[ "${AITER_JIT_TARGET:-}" != /* ]]; then
    ...
    warn "  Set AITER_JIT_TARGET in qwen38-27b.env to the jit/ directory from that path."
else
...
# the other way costs a full weight load before the server dies on an unknown
# argument.
#
...
if [[ -n "$SPECULATIVE" ]]; then
  ...
  REPLAYSSM_SPEC=0."
    ...
    # ── The sampling landmines — FAIL CLOSED HERE ───────────────────────────
    #
    # These do NOT fail at startup. The server comes up healthy and serves
    # correctly until the first request that trips one, and then the SCHEDULER
    # dies and takes every concurrent request with it.
    #
    # The 2.4T bundle warns and continues, because speculative decoding is off
    # by default there and switching it on is a deliberate act. Here NEXTN is
    # the DEFAULT, so a warning would mean the out-of-the-box configuration is
    # one ordinary request away from dying. So this dies instead — and names
    # SPEC_GATE=off for anyone who has measured the risk and accepts it.
    spec_unsafe=""
    if ! renorm_has probed; then
        ...
        warn "  Proceeding — an unreadable probe is not a failed probe. Run"
        warn "  './serve-qwen38-27b.sh speccheck' to investigate."
        ...
        if renorm_has p; then
            spec_unsafe="top_k: this image has the HIP top_p renorm kernel but not top_k.
  A request setting TOP_K raises NameError and KILLS THE SERVER."
        else
            spec_unsafe="top_p and top_k: this image has neither HIP renorm kernel.
  A request setting TOP_P (<1.0) or TOP_K raises
  \"TypeError: 'NoneType' object is not callable\" and KILLS THE SERVER.
  One such request poisons its whole batch."
        fi
    fi

    # The second landmine, and the one a real client is far likelier to hit:
    # top_p/top_k are optional, temperature > 0 is what every chat client sends
    # — and it is Qwen's own recommended default for this model.
    if renorm_has v && ! renorm_has t; then
        spec_unsafe="${spec_unsafe:+$spec_unsafe

}temperature: this image's is_hip() branch claims non-greedy verify is
  available but never binds tree_speculative_sampling_target_only, and that
  kernel does not exist on ROCm at all. A request with TEMPERATURE > 0 raises
  \"NameError: tree_speculative_sampling_target_only\" and KILLS THE SERVER
  (sglang #33694, fixed 6 Aug 2026)."
    fi

    if [[ -n "$spec_unsafe" ]]; then
        if [[ "$SPEC_GATE" == "off" ]]; then
            warn "SPEC_GATE=off — starting anyway with KNOWN-UNSAFE speculative decoding:"
            warn "$spec_unsafe"
        else
            die "This image cannot safely run speculative decoding, and SPECULATIVE=$SPECULATIVE.

  $spec_unsafe

  READ THIS TWICE FOR QWEN3.8. Qwen's own recommended sampling for this model
  is temperature=1.0, top_p=0.95, top_k=20 — so the DOCUMENTED settings send
  exactly the affected parameters. You will not have to go looking for this
  bug; a normal client finds it on the first request, and it takes the server
  down rather than the request.

  Three ways forward, best first:
    1. Pull a newer lmsysorg/sglang-rocm:*-mi35x-* image into a SECOND
       SIF_PATH and run './serve-qwen38-27b.sh speccheck' against it.
    2. SPECULATIVE=none in qwen38-27b.env — safe, and costs you the MTP
       speedup rather than the server.
    3. SPEC_GATE=off — start anyway. Only do this if you control every client
       and can guarantee greedy sampling (temperature=0, no top_p/top_k)."
        fi
    fi
    ...
    if [[ "$SPECULATIVE" == "dspark" && "${DRAFT_ROPE_TYPE:-}" == "default" \
        ...
        if [[ -z "$CONTEXT_LEN" ]] \
           || { [[ "$CONTEXT_LEN" =~ ^[0-9]+$ ]] && (( CONTEXT_LEN > DRAFT_TRAINED_CTX )); }; then
            ...
            warn "  './bench-qwen38-27b.sh longcontext' before trusting it in an agentic session."
        fi
    fi
fi
...
# ── GDN state pool ──────────────────────────────────────────────────────────
#
# 48 of Qwen3.8-27B's 64 layers are Gated DeltaNet (full_attention_interval: 4),
# and their recurrent state lives in its own pool. Per layer per state slot:
# a 10240-wide causal conv of kernel 4, plus a 48 x 128 x 128 SSM state, in
# float32 = 3.156 MiB. Across 48 layers that is 151.5 MiB = 0.148 GiB PER SLOT,
# and a request holds several slots (5 under extra_buffer, 3 under no_buffer).
# That pool — not KV — is what caps concurrency here. Every one of these is
# empty by default, so the argv is unchanged until you set one.
mamba_flags=()
...
if (( ${#mamba_flags[@]} )); then
  ...
  Unset the matching variables in qwen38-27b.env, or use a newer image."
    fi
...
memfrac_flag=()
if [[ "$MEM_FRACTION" == "auto" ]]; then
    log "MEM_FRACTION=auto — passing no --mem-fraction-static; SGLang derives one
  and THEN cuts it ~7.5% for the vision tower (adjust_mem_fraction_for_vlm runs
  only on this path). An explicit 0.9 usually leaves you with MORE."
else
    memfrac_flag=(--mem-fraction-static "$MEM_FRACTION")
fi

# ── Multimodal ──────────────────────────────────────────────────────────────
# Vision is on by default.
#
# NOTE THE FLAG. --enable-multimodal is Optional[bool] but SGLang's arg builder
# unwraps Optional and renders any bool as action="store_true" (arg_utils.py,
# "Bool -> store_true"). So it takes NO VALUE — '--enable-multimodal False'
# would hand argparse a stray positional and fail at startup, and there is no
# way to say "off" through it at all.
#
# The flag that actually turns the tower off is --language-model-only: "Skip
# the multimodal encoder entirely: its weights are never loaded and the tower
# is never built, freeing that GPU memory for KV cache. Multimodal requests are
# rejected."
mm_flags=()
if [[ "$ENABLE_MULTIMODAL" == "0" ]]; then
    mm_rc=0; has_flag '--language-model-only' || mm_rc=$?
    if (( mm_rc == 1 )); then
        warn "ENABLE_MULTIMODAL=0 but this image has no --language-model-only flag;
  serving with the vision tower loaded. Use a newer image, or accept vision on."
    else
        mm_flags=(--language-model-only)
        log "Multimodal DISABLED — text only. The BF16 vision tower (0.86 GiB) is never
  built, and that memory goes to KV instead. Multimodal requests are rejected
  rather than silently mishandled."
    fi
else
    log "Multimodal ON (vision tower BF16). Verify end-to-end with './serve-qwen38-27b.sh vlmcheck'."
fi

# ── Buffered ReplaySSM decode ───────────────────────────────────────────────
# The throughput branch. Validated far above (mutually exclusive with the
# spec-verify variant, needs no_buffer + Triton decode); here we only check the
# image actually offers the flag before committing to a load.
if [[ "$REPLAYSSM_DECODE" == "1" ]]; then
    rsd_rc=0; has_flag '--enable-linear-replayssm' || rsd_rc=$?
    if (( rsd_rc == 1 )); then
        die "This image has no --enable-linear-replayssm. It is present in mainline
  server_args.py, so an image without it predates the feature — pull a newer
  lmsysorg/sglang-rocm:*-mi35x-* tag into a SECOND SIF_PATH and test there, or
  set REPLAYSSM_DECODE=0."
    fi
    (( rsd_rc == 2 )) && warn "  Could not read --help from the image; passing --enable-linear-replayssm unverified."
    perf_flags+=(--enable-linear-replayssm)
    [[ -n "$REPLAYSSM_CACHE_LEN" ]] && perf_flags+=(--linear-replayssm-cache-len "$REPLAYSSM_CACHE_LEN")
    log "Buffered ReplaySSM decode ENABLED. Upstream claims ~1.2-1.5x GDN decode"
    log "  bandwidth at batch >= 64 — so it is a THROUGHPUT lever, not a latency one,"
    log "  and it does nothing for a single-stream benchmark. Measure at concurrency."
fi

# ── aiter ───────────────────────────────────────────────────────────────────
# SGLANG_USE_AITER=1 is the ONE env var the verified MI355X cell sets. This is
# where the gfx950 MXFP4 speed comes from.
aiter_env=()
...
if [[ -n "$loader_cfg" || -n "$LOAD_FORMAT" ]]; then
  ...
  Set WEIGHT_LOAD_THREADS=0 in qwen38-27b.env to silence this."
            loader_cfg=""
        fi
        if [[ -n "$LOAD_FORMAT" ]] && ! grep -q -- "$LOAD_FORMAT" <<<"$help_text"; then
            die "This image's --load-format does not offer '$LOAD_FORMAT'.
  Run './serve-qwen38-27b.sh loadstat' to see what it does support, then set
  LOAD_FORMAT in qwen38-27b.env accordingly (empty = the image's default)."
        fi
    fi
fi
...
cmd=("${launcher[@]}"
     ...
     ${memfrac_flag[@]+"${memfrac_flag[@]}"}
     ${cgraph_flag[@]+"${cgraph_flag[@]}"}
     --host 0.0.0.0 --port "$PORT"
     --api-key "$Q27X_API_KEY"
...
if [[ "$ROCM_MODE" != "auto" ]]; then
  ...
  container    : $PROBE_ERR
  Run './serve-qwen38-27b.sh gpucheck' — it tries every mode and names the cause.
...
if [[ -z "$chosen_mode" ]]; then
  ...
  Run './serve-qwen38-27b.sh gpucheck' for the full report."
    fi
  ...
  node's ROCm. Run './serve-qwen38-27b.sh gpucheck' for the full report, and see
...
# after the weight load. Catch a read-only path in seconds instead.
if [[ "${AITER_JIT_TARGET:-}" == /* ]]; then
  ...
  Fix the bind, or set AITER_JIT_TARGET in qwen38-27b.env to the jit/ directory
...
while true; do
  ...
  Set WEIGHT_LOAD_THREADS=8 in qwen38-27b.env. See './serve-qwen38-27b.sh loadstat'."
        fi
        break
    fi
    if ! kill -0 "$SERVER_PID" 2>/dev/null; then
     ...
     Unknown architecture?   ./serve-qwen38-27b.sh check
     Unknown parser name?    ./serve-qwen38-27b.sh parsers
     KV/GDN pool OOM?        set CONTEXT_LEN=131072 in qwen38-27b.env and retry
...
============================================================================
  ...
  API key:     $API_KEY_FILE
               export Q27X_API_KEY="\$(cat $API_KEY_FILE)"
  ...
  Benchmark:  ./bench-qwen38-27b.sh sweep
              (no published MI355X figures exist yet — these are the first)

  Stop with Ctrl-C, 'scancel $JOBID', or './serve-qwen38-27b.sh stop'.
============================================================================
...
if [[ "$DETACH" -eq 1 ]]; then
    ...
    # (until './serve-qwen38-27b.sh stop' or the SLURM job/allocation ends).
    trap - EXIT INT TERM
    disown "$SERVER_PID" 2>/dev/null || true
    log "Detached. You have your shell back — the server keeps running on this node."
    log "  Logs:  tail -f $LOG_FILE"
    log "  Stop:  ./serve-qwen38-27b.sh stop"
    exit 0
fi
